...
from openhire.cron.types import CronJob, CronPayload, CronSchedule
...
def test_employee_id_persists_on_cron_payload(tmp_path) -> None:
    ...
    service.update_job(job.id, message="更新后的检查")

    fresh = CronService(store_path)
    [loaded] = fresh.list_jobs(include_disabled=True)
    assert loaded.payload.employee_id == "nova-fe"
    assert loaded.payload.message == "更新后的检查"
