...
from openhire.session.manager import Session
...
@pytest.mark.asyncio
async def test_process_message_schedules_skill_learning_after_complex_completed_turn(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    loop._skill_proposal_generator.submit_turn = AsyncMock(return_value=None)  # type: ignore[union-attr]
    scheduled = []
    ...
    for idx in range(5):
        tool_messages.extend([
            ...
            {"role": "tool", "tool_call_id": f"call_{idx}", "name": "read_file", "content": "ok"},
        ])
    loop._run_agent_loop = AsyncMock(return_value=(
        "done",
        ["read_file"] * 5,
    ...
    result = await loop._process_message(
        InboundMessage(channel="feishu", sender_id="u1", chat_id="complex", content="complex task")
    )
    ...
    for coro in scheduled:
        await coro
    loop._skill_proposal_generator.submit_turn.assert_awaited_once()  # type: ignore[union-attr]
    kwargs = loop._skill_proposal_generator.submit_turn.await_args.kwargs  # type: ignore[union-attr]
    assert kwargs["trigger_reasons"] == ["complex_task_5_tool_calls"]
    assert kwargs["session_key"] == "feishu:complex"
    assert kwargs["final_content"] == "done"
