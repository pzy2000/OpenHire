...
SCRIPT_DIR = Path("openhire/skills/skill-creator/scripts").resolve()
...
def test_validate_skill_accepts_existing_skill_creator() -> None:
    valid, message = quick_validate.validate_skill(
        Path("openhire/skills/skill-creator").resolve()
    )
...
def test_validate_skill_accepts_catalog_and_hermes_metadata_fields(tmp_path: Path) -> None:
    ...
    (skill_dir / "SKILL.md").write_text(
        ...
        "name: metadata-skill\n"
        "description: Valid description\n"
        "homepage: https://example.com/skills/metadata-skill\n"
        "version: 1.2.3\n"
        "author: OpenHire\n"
        "category: devops\n"
