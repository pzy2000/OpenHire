...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("openhire") / "templates"
...
def test_system_prompt_contains_multi_docker_role_summary_rule(tmp_path) -> None:
    """System prompt should define multi-docker role summary behavior."""
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)

    prompt = builder.build_system_prompt()
    assert ">=2 docker_agent" in prompt
    assert "终稿前汇总" in prompt
    assert "每个角色 1-2 句" in prompt
    assert "失败保留并继续" in prompt
