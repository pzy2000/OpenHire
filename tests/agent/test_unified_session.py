...
from openhire.command.builtin import cmd_new, register_builtin_commands
from openhire.command.router import CommandContext, CommandRouter
from openhire.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("openhire.agent.loop.SessionManager"), \
         patch("openhire.agent.loop.Dream"):
        loop = AgentLoop(
            bus=bus,
            provider=provider,
            workspace=tmp_path,
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        ...
        from openhire.config.loader import save_config
