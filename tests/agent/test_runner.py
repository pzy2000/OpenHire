...
from openhire.agent.tools.base import Tool
from openhire.agent.tools.registry import ToolRegistry
from openhire.providers.base import LLMResponse, ToolCallRequest
...
async def test_runner_calls_hooks_in_order():
    from openhire.agent.hook import AgentHook, AgentHookContext
    from openhire.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".openhire" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from openhire.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".openhire" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "openhire.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "openhire.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("openhire.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "openhire.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("openhire.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from openhire.agent.runner import (
        AgentRunSpec,
        AgentRunner,
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from openhire.agent.loop import AgentLoop
    from openhire.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from openhire.bus.events import InboundMessage
    from openhire.bus.queue import MessageBus
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from openhire.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from openhire.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from openhire.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from openhire.agent.loop import AgentLoop
    from openhire.agent.runner import _BACKFILL_CONTENT
    from openhire.bus.events import InboundMessage
    from openhire.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from openhire.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from openhire.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from openhire.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTIONS_PER_TURN
    from openhire.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from openhire.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTION_CYCLES
    from openhire.bus.events import InboundMessage
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    from openhire.agent.loop import UNIFIED_SESSION_KEY
    from openhire.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from openhire.bus.events import InboundMessage
    from openhire.bus.queue import MessageBus
    from openhire.agent.runner import _MAX_INJECTIONS_PER_TURN
...
async def test_drain_injections_on_empty_final_response():
    ...
    from openhire.agent.runner import AgentRunSpec, AgentRunner, _MAX_EMPTY_RETRIES
    from openhire.bus.events import InboundMessage
...
async def test_drain_injections_set_flag_when_followup_arrives_after_last_iteration():
    ...
    from openhire.agent.hook import AgentHook
    from openhire.agent.runner import AgentRunSpec, AgentRunner
    from openhire.bus.events import InboundMessage
