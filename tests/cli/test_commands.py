...
from openhire.cli.commands import _make_provider, app
from openhire.config.schema import Config
from openhire.cron.types import CronJob, CronPayload
from openhire.providers.openai_codex_provider import _strip_model_prefix
from openhire.providers.registry import find_by_name
...
def mock_paths():
    ...
    with patch("openhire.config.loader.get_config_path") as mock_cp, \
         patch("openhire.config.loader.save_config") as mock_sc, \
         patch("openhire.config.loader.load_config") as mock_lc, \
         patch("openhire.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "OpenHire is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from openhire.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "openhire.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openhire.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'openhire agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"openhire gateway --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    from openhire.providers.openai_compat_provider import OpenAICompatProvider

    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from openhire.cli.commands import _make_provider
    from openhire.config.schema import Config
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from openhire.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("openhire.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("openhire.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("openhire.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("openhire.cli.commands._make_provider", return_value=object()), \
         patch("openhire.cli.commands._print_agent_response") as mock_print_response, \
         patch("openhire.bus.queue.MessageBus"), \
         patch("openhire.cron.service.CronService"), \
         patch("openhire.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "openhire.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("openhire.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openhire.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openhire.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openhire.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("openhire.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openhire.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("openhire.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("openhire.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openhire.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openhire.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openhire.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("openhire.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openhire.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("openhire.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openhire.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("openhire.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "openhire.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("openhire.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openhire.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "openhire.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "openhire.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("openhire.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("openhire.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("openhire.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("openhire.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openhire.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("openhire.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openhire.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openhire.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr("openhire.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("openhire.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("openhire.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openhire.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "openhire.utils.evaluator.evaluate_response",
...
def test_gateway_cron_skips_delivery_when_openhire_employee_sent_message(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    ...
    class _FakeAgentLoop:
        def __init__(self, *args, **kwargs) -> None:
            self.model = "test-model"
            self.tools = {"openhire": _SentTool()}

        async def process_direct(self, *_args, **_kwargs):
            return OutboundMessage(
                channel="feishu",
                chat_id="oc_group",
                content="employee already sent",
            )
    ...
    monkeypatch.setattr("openhire.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openhire.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr("openhire.utils.evaluator.evaluate_response", AsyncMock(return_value=True))
    ...
    job = CronJob(
        ...
        payload=CronPayload(
            message="Delegate c15e1974 to send a report.",
            deliver=True,
            channel="feishu",
            to="oc_group",
        ),
    ...
    assert response == "employee already sent"
    bus.publish_outbound.assert_not_awaited()


def test_gateway_employee_owned_cron_delegates_to_employee(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    ...
    class _EmployeeTool:
        ...
        def set_context(self, channel: str, chat_id: str, message_id: str | None = None) -> None:
            seen["context"] = (channel, chat_id, message_id)

        def start_turn(self) -> None:
            self._sent_in_turn = False

        async def execute(self, **kwargs):
            seen["delegate"] = kwargs
            return "delegated report"

    class _FakeAgentLoop:
        def __init__(self, *args, **kwargs) -> None:
            self.model = "test-model"
            self.tools = {"openhire": _EmployeeTool()}

        async def process_direct(self, *_args, **_kwargs):
            raise AssertionError("employee-owned cron should not run through the main agent")
    ...
    assert response == "delegated report"
    assert seen["context"] == ("feishu", "oc_group", None)
    assert seen["delegate"]["action"] == "delegate"
    assert seen["delegate"]["agent_id"] == "nova-fe"
    assert "Task 'employee-report' has been triggered." in seen["delegate"]["task"]
    bus.publish_outbound.assert_not_awaited()
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from openhire.cli.commands import _migrate_cron_store
    ...
    with patch("openhire.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_cli_host_overrides_configured_host(monkeypatch, tmp_path: Path) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    result = runner.invoke(app, ["gateway", "--config", str(config_file), "--host", "0.0.0.0"])
