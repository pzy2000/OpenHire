...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/openhire/openhire"
    assert headers["X-OpenRouter-Title"] == "openhire"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://openhire.ai",
    ...
    headers = MockClient.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://openhire.ai"
...
def test_openai_compat_merges_consecutive_assistant_reasoning_fields() -> None:
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI"):
        provider = OpenAICompatProvider()
    ...
    assert sanitized[1]["content"] == "first\n\nsecond"
    assert sanitized[1]["reasoning_content"] == "reasoning 1\n\nreasoning 2"


def test_openai_compat_backfills_missing_reasoning_content_for_deepseek_v4_tool_turn() -> None:
    spec = find_by_name("deepseek")
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI"):
        provider = OpenAICompatProvider(api_key="k", default_model="deepseek-v4-flash", spec=spec)
    ...
    assistant = next(msg for msg in kwargs["messages"] if msg.get("role") == "assistant")
    assert assistant["reasoning_content"] == ""


def test_openai_compat_backfills_missing_reasoning_content_when_history_is_thinking() -> None:
    spec = find_by_name("openai")
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI"):
        provider = OpenAICompatProvider(api_key="k", default_model="gpt-5.4", spec=spec)
    ...
    kwargs = provider._build_kwargs(
        messages=[
            ...
            {"role": "tool", "tool_call_id": "call_b", "name": "exec", "content": "ok"},
            {"role": "user", "content": "continue"},
        ],
        tools=None,
        model="gpt-5.4",
    ...
    assistant_messages = [m for m in kwargs["messages"] if m.get("role") == "assistant"]
    assert assistant_messages[0]["reasoning_content"] == "kept reasoning"
    assert assistant_messages[1]["reasoning_content"] == ""


def test_openai_compat_does_not_backfill_reasoning_content_for_plain_openai() -> None:
    spec = find_by_name("openai")
    with patch("openhire.providers.openai_compat_provider.AsyncOpenAI"):
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("OPENHIRE_STREAM_IDLE_TIMEOUT_S", "0")
