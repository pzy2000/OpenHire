...
from openhire.channels.matrix import _build_matrix_text_content

import openhire.channels.matrix as matrix_module
from openhire.bus.events import OutboundMessage
from openhire.bus.queue import MessageBus
from openhire.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from openhire.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("openhire.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "openhire.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("openhire.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "openhire.channels.matrix.asyncio.create_task", _fake_create_task
    )
