...
from openhire.config.paths import get_media_dir, get_runtime_subdir
from openhire.config.schema import Base
from openhire.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.openhire/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'openhire channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"openhire-{uuid.uuid4().hex[:12]}"
