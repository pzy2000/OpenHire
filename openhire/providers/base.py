...
from openhire.utils.helpers import image_placeholder_text
...
class LLMProvider(ABC):
    ...
    def _enforce_role_alternation(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        def merge_assistant_metadata(prev: dict[str, Any], curr: dict[str, Any]) -> None:
            """Preserve provider continuation metadata when assistant turns merge."""
            if "reasoning_content" in curr:
                curr_reasoning = curr.get("reasoning_content")
                prev_reasoning = prev.get("reasoning_content")
                if curr_reasoning is None:
                    pass
                elif not prev_reasoning:
                    prev["reasoning_content"] = curr_reasoning
                elif (
                    isinstance(prev_reasoning, str)
                    and isinstance(curr_reasoning, str)
                    and curr_reasoning
                    and curr_reasoning != prev_reasoning
                ):
                    prev["reasoning_content"] = f"{prev_reasoning}\n\n{curr_reasoning}"

            curr_thinking = curr.get("thinking_blocks")
            if isinstance(curr_thinking, list) and curr_thinking:
                prev_thinking = prev.get("thinking_blocks")
                if isinstance(prev_thinking, list):
                    prev["thinking_blocks"] = [*prev_thinking, *curr_thinking]
                else:
                    prev["thinking_blocks"] = list(curr_thinking)

            if "extra_content" in curr and "extra_content" not in prev:
                prev["extra_content"] = curr["extra_content"]
        ...
        for msg in messages:
            ...
            if (
                ...
                if isinstance(prev_content, str) and isinstance(curr_content, str):
                    ...
                    if role == "assistant":
                        merge_assistant_metadata(prev, msg)
                else:
                    merged[-1] = dict(msg)
            else:
