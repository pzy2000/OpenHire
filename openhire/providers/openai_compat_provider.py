...
from openhire.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from openhire.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from openhire.providers.registry import ProviderSpec
...
_REASONING_ROUNDTRIP_MODEL_MARKERS = (
    "deepseek-v4",
    "deepseek-reasoner",
    "deepseek-r1",
    "kimi-k2",
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/openhire/openhire",
    "X-OpenRouter-Title": "openhire",
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _reasoning_roundtrip_active(
        self,
        messages: list[dict[str, Any]],
        model: str | None,
        reasoning_effort: str | None,
    ) -> bool:
        """Return True when assistant tool-call messages must carry reasoning_content.

        DeepSeek V4 thinking mode defaults to enabled, and compatible gateways
        may sit behind a generic provider name.  If the current history already
        contains reasoning_content, keep the round-trip contract active even
        when the configured model name is opaque.
        """
        if any(
        ...
        if reasoning_effort and reasoning_effort.lower() not in {"none", "minimal"}:
            return True

        model_name = (model or self.default_model or "").lower()
        spec_name = self._spec.name if self._spec else ""
        if spec_name == "deepseek" and ("v4" in model_name or "reasoner" in model_name):
            return True
        return any(marker in model_name for marker in _REASONING_ROUNDTRIP_MODEL_MARKERS)
    ...
    def _ensure_reasoning_content_for_tool_calls(
        ...
        """Backfill missing reasoning_content keys for thinking tool-call turns."""
        if not self._reasoning_roundtrip_active(messages, model, reasoning_effort):
            return messages
    ...
    def _prepare_chat_messages(
        ...
        return self._ensure_reasoning_content_for_tool_calls(
            sanitized,
            model,
            reasoning_effort,
        )
    ...
    def _build_responses_body(
        ...
        model_name = model or self.default_model
        sanitized_messages = self._prepare_chat_messages(
            messages,
            model_name,
            reasoning_effort,
        )
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("OPENHIRE_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
