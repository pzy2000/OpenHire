...
async def connect_mcp_servers(
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        # Guard protocol-level calls so a non-responsive MCP server cannot
        # block the agent loop indefinitely during startup.
        connect_timeout = max(int(getattr(cfg, "connect_timeout", 20) or 20), 1)
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "MCP server '{}': initialize handshake timed out after {}s, giving up",
                    name,
                    connect_timeout,
                )
                await server_stack.aclose()
            ...
            try:
                ...
                logger.error(
                    "MCP server '{}': list_tools timed out after {}s, giving up",
                    name,
                    connect_timeout,
                )
                await server_stack.aclose()
            ...
            try:
                resources_result = await asyncio.wait_for(
                    session.list_resources(), timeout=connect_timeout
                )
            ...
            try:
                prompts_result = await asyncio.wait_for(
                    session.list_prompts(), timeout=connect_timeout
                )
